        ...
        if acao == "adicionar":
            if len(comando) > 1:
                ...
                if comando[-1].isdigit():
                    descricao = " ".join(comando[1:-1]) 
                    # Junta as palavras de 1 até o penúltimo item
                    prioridade = comando[-1]
                    # A última palavra é a prioridade
                else:
                    descricao = " ".join(comando[1:])
                    # Junta todas as palavras após o 'adicionar' como descrição
